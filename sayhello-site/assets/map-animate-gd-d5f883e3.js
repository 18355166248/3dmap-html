import { _ as $, a as H } from "./animate2-2f11d126.js";
import {
  F as X,
  G as M,
  V as g,
...
import { D as ue } from "./index-4ec0cc76.js";
import { G as he } from "./Grid-77f5dd1e.js";
import { L as pe } from "./Label3d-1a598e21.js";
import { G as I, P as R } from "./GradientShader-7cc661aa.js";
import { g as m } from "./index-4db78ffb.js";
import { c as fe, s as me } from "./chinaBlurLine-b7b06be6.js";
import { o as ge } from "./ocean-bg-19f8644c.js";
import {
  h as ve,
  r as ye,
  g as we,
...
import { w as Ce } from "./szxs_logo-02219344.js";
import { t as N } from "./utils-9af1928d.js";
import { L as Me, a as Se } from "./Line2-7598ed88.js";
...
class ze {
  ...
  init() {
    (this.instance = new se()),
      ...
      this.instance.on("onProgress", (r, e, n) => {
        ((e / n) * 100).toFixed(2) + "";
      }),
    ...
    let t = "/sayhello-site/",
      a = [
        { type: "Texture", name: "huiguang", path: ve },
        { type: "Texture", name: "watermark", path: Ce },
        { type: "Texture", name: "rotationBorder1", path: ye },
        { type: "Texture", name: "rotationBorder2", path: xe },
        { type: "Texture", name: "guangquan1", path: we },
        { type: "Texture", name: "guangquan2", path: be },
        { type: "Texture", name: "chinaBlurLine", path: fe },
        { type: "Texture", name: "ocean", path: ge },
        { type: "Texture", name: "side", path: me },
        {
          type: "File",
          name: "guangdong",
...
class Te {
  constructor({ assets: t, time: a }, r = {}) {
    ...
    let e = N(this.config.data);
    this.create(e), console.log(this.mapGroup);
  }
  geoProjection(t) {
    return T().center(this.config.center).scale(120).translate([0, 0])(t);
  }
  create(t) {
    t.features.forEach((a) => {
      const r = new q();
      let { name: e, center: n = [], centroid: i = [] } = a.properties;
      this.coordinates.push({ name: e, center: n, centroid: i });
      const d = {
      ...
      let o = [this.config.topFaceMaterial, this.config.sideMaterial];
      a.geometry.coordinates.forEach((l) => {
        l.forEach((c, s) => {
          const p = new j();
          for (let f = 0; f < c.length; f++) {
            if (!c[f][0] || !c[f][1]) return !1;
            const [y, G] = this.geoProjection(c[f]);
            f === 0 && p.moveTo(y, -G), p.lineTo(y, -G);
          }
          const u = new Q(p, d),
            h = new b(u, o);
          r.add(h);
  ...
  createMaterial() {
    ...
    t.onBeforeCompile = (e) => {
      (e.uniforms = {
        ...e.uniforms,
        uColor1: { value: new x(2781042) },
        uColor2: { value: new x(860197) },
      }),
    ...
    let r = new O({ color: 16777215, map: a, fog: !1, opacity: 1, side: S });
    return (
      this.time.on("tick", () => {
        a.offset.y += 1e-4;
      }),
  ...
  create(t) {
    ...
    if (
      (t.features.forEach((e) => {
        const n = new q();
        let { name: i, center: d = [], centroid: o = [] } = e.properties;
        this.coordinates.push({ name: i, center: d, centroid: o }),
          (n.userData.name = i),
          e.geometry.coordinates.forEach((l) => {
            l.forEach((c) => {
              const s = new j();
              for (let u = 0; u < c.length; u++) {
                if (!c[u][0] || !c[u][1]) return !1;
                const [h, f] = this.geoProjection(c[u]);
                u === 0 && s.moveTo(h, -f), s.lineTo(h, -f);
              }
              const p = new W(s);
              if (a) r.push(p);
              else {
                const u = new b(p, this.config.material);
                (u.userData.name = i),
...
class z {
    ...
    for (let i = 0; i < e.length; i++) {
      ...
      d.properties.name !== r &&
        d.geometry.coordinates.forEach((o) => {
          ...
          a === "Line2"
            ? (o[0].forEach((s) => {
                const [p, u] = this.geoProjection(s);
                l.push(p, -u, 0);
              }),
              (c = this.createLine2(l)))
            : o[0].forEach((s) => {
                const [p, u] = this.geoProjection(s);
                l.push(new g(p, -u, 0)), (c = this.createLine(l));
              }),
            n.add(c);
        });
    }
  ...
  createLine(t) {
    const { material: a, renderOrder: r, type: e } = this.config,
      n = new Y();
    n.setFromPoints(t);
    let i = new J(n, a);
...
class Fe extends de {
  constructor(t, a) {
    super(t, a),
      ...
      (this.scene.fog = new K(596769, 1, 50)),
      (this.scene.background = new x(596769)),
      ...
      (this.interactionManager = new Ge(
      ...
      (this.assets = new ze(() => {
        ...
        r.addLabel("focusMap", 3.5),
          ...
          r.addLabel("bar", 5),
          r.add(
            m.to(this.camera.instance.position, {
              duration: 2.5,
              delay: 2,
          ...
          r.add(
            m.to(this.focusMapGroup.position, {
              duration: 1,
              x: 0,
              y: 0,
              z: 0,
          ...
          r.add(
            m.to(this.focusMapSideMaterial, {
              duration: 1,
              opacity: 1,
              ease: "circ.out",
              onComplete: () => {
          ...
          this.otherLabel.map((e, n) => {
            let i = e.element.querySelector(".other-label");
            r.add(
              m.to(i, {
                duration: 1,
                delay: 0.1 * n,
          ...
          r.add(
            m.to(this.guangdongLineMaterial, {
              duration: 0.5,
              delay: 0.3,
              opacity: 1,
            }),
          ...
          this.allProvinceLabel.map((e, n) => {
            let i = e.element.querySelector(".provinces-label-wrap"),
              d = e.element.querySelector(".number .value"),
              o = Number(d.innerText),
              l = { score: 0 };
            r.add(
              m.to(i, {
                duration: 1,
            ...
            function s() {
              d.innerText = l.score.toFixed(0);
            }
            r.add(c, "bar");
          }),
          this.allGuangquan.map((e, n) => {
            r.add(
              m.to(e.children[0].scale, {
                duration: 1,
                delay: 0.1 * n,
                x: 1,
                y: 1,
  ...
  initEnvironment() {
    let t = new Z(16777215, 3);
    this.scene.add(t);
    let a = new ee(16777215, 8);
    if (
  ...
  createPost() {
    let t = { _Saturation: 1, _Brightness: 1, _Contrast: 1 };
    const a = new Le(this.renderer.instance);
    a.addPass(new Pe(this.scene, this.camera.instance));
    const r = new _e(Re);
    if (
      ((r.uniforms._Brightness.value = t._Brightness),
      (r.uniforms._Saturation.value = t._Saturation),
      (r.uniforms._Contrast.value = t._Contrast),
      a.addPass(r),
      (this.composer = a),
      ...
      const e = this.debug.instance.addFolder("postprocessing");
      e.add(t, "_Brightness", -3, 3, 0.01).onChange((n) => {
        r.uniforms._Brightness.value = Number(n);
      }),
        e.add(t, "_Saturation", -3, 3, 0.01).onChange((n) => {
          r.uniforms._Saturation.value = Number(n);
        }),
        e.add(t, "_Contrast", -3, 3, 0.01).onChange((n) => {
          r.uniforms._Contrast.value = Number(n);
        }),
        e.add(this.renderer, "postprocessing");
    }
  }
  createPointLight(t) {
    const a = new ae(1924702, t.intensity, t.distance, 1);
  ...
  createModel() {
    let t = new M(),
      a = new M();
    this.focusMapGroup = a;
    let { china: r, chinaTopLine: e, chinaBottomLine: n } = this.createChina(),
      {
        guangdong: i,
        guangdongTop: d,
        guangdonLine: o,
  ...
  createChina() {
    let t = this.assets.instance.getResource("china"),
      a = new D(this, {
        ...
        merge: !1,
        material: new U({ color: 1190450, transparent: !0, opacity: 1 }),
        renderOrder: 2,
      }),
      r = new z(this, {
        center: this.pointCenter,
    ...
    return (
      ...
      { china: a, chinaTopLine: r, chinaBottomLine: e }
    );
  }
  createProvince() {
    let t = this.assets.instance.getResource("guangdong"),
      [a, r] = this.createProvinceMaterial();
    (this.focusMapTopMaterial = a), (this.focusMapSideMaterial = r);
    let e = new Te(this, {
        center: this.pointCenter,
      ...
      n = new O({ color: 16777215, transparent: !0, opacity: 0.5 });
    new I(n),
      (this.defaultMaterial = n),
    ...
    let i = new D(this, {
      center: this.pointCenter,
      position: new g(0, 0, 0.72),
      data: t,
      material: n,
      renderOrder: 2,
    });
    i.mapGroup.children.map((o) => {
      o.children.map((l) => {
    ...
    return (
      (d.lineGroup.position.z += 0.73),
      { guangdong: e, guangdongTop: i, guangdonLine: d }
    );
  }
  createProvinceMaterial() {
    ...
    let a = this.assets.instance.getResource("side");
    (a.wrapS = w), (a.wrapT = w), a.repeat.set(1, 1.5), (a.offset.y += 0.065);
    let r = new O({ color: 16777215, map: a, fog: !1, opacity: 0, side: S });
    return (
      this.time.on("tick", () => {
        a.offset.y += 0.005;
      }),
    ...
    this.eventElement.map((e) => {
      this.interactionManager.add(e),
        e.addEventListener("mousedown", (n) => {
          console.log(n.target.userData.name);
        }),
        ...
        e.addEventListener("mouseout", (n) => {
          (t = t.filter(
            (i) => i.userData.name !== n.target.parent.userData.name
          )),
  ...
  createBar() {
    ...
    (this.allBar = []),
      ...
      a.map((o, l) => {
        ...
        const p = new ne(0.1 * e, 0.1 * e, c);
        p.translate(0, 0, c / 2);
        const u = new b(p, s);
        u.renderOrder = 5;
        let h = u,
          [f, y] = this.geoProjection(o.centroid);
        h.position.set(f, -y, 0.95), h.scale.set(1, 1, 0);
        let G = this.createQuan(new g(f, 0.94, y), l),
          k = this.createHUIGUANG(c, l > 3 ? 16776948 : 7863285);
        h.add(...k), r.add(h), (r.rotation.x = -Math.PI / 2);
        let V = d(o, l, new g(f, -y, 1.6 + c));
        this.allBar.push(h),
          this.allBarMaterial.push(s),
          this.allGuangquan.push(G),
          this.allProvinceLabel.push(V);
      }),
      this.scene.add(r);
    function d(o, l, c) {
      let s = t.label3d.create("", "provinces-label", !1);
      return (
        s.init(
      ...
      <div class="provinces-label-wrap">
        ...
        t.label3d.setLabelStyle(s, 0.01, "x"),
        s.setParent(t.labelGroup),
        s
      );
    }
  }
  createHUIGUANG(t, a) {
    ...
    let n = new v({
      ...
      i = new b(r, n);
    (i.renderOrder = 10), i.rotateX(Math.PI / 2);
    let d = i.clone(),
      o = i.clone();
    return (
  ...
  createAnimateVideo() {
    this.createAnimateVideoItem(".map-gd-video1", new g(11, 0.4, 1)),
      this.createAnimateVideoItem(".map-gd-video2", new g(-11, 0.4, 2));
  }
  ...
  createLabel() {
    ...
    e.push(n), e.push(i), e.push(d), (this.otherLabel = e);
    function o(s, p, u) {
      let h = p.create("", `china-label ${s.blur ? " blur" : ""}`, !1);
      const [f, y] = t.geoProjection(s.center);
      return (
        h.init(
          `<div class="other-label"><img class="label-icon" src="${E}">${s.name}</div>`,
          new g(f, -y, 0.4)
        ),
        p.setLabelStyle(h, 0.02, "x"),
        h.setParent(u),
        h
      );
    }
    function l(s, p, u) {
      let h = p.create("", "guangdong-label", !1);
      const [f, y] = t.geoProjection(s.center);
      return (
        h.init(
          `<div class="other-label"><span>${s.name}</span><span>${s.enName}</span></div>`,
          new g(f, -y, 0.4)
        ),
        p.setLabelStyle(h, 0.02, "x"),
        h.setParent(u),
    ...
    function c(s, p, u) {
      let h = p.create(
        "",
        `decoration-label  ${s.reflect ? " reflect" : ""}`,
        !1
      );
      const [f, y] = t.geoProjection(s.center);
      return (
        h.init(
          `<div class="other-label"><img class="label-icon" style="width:${s.width};height:${s.height}" src="${s.icon}">`,
          new g(f, -y, 0.4)
        ),
        p.setLabelStyle(h, 0.02, "x"),
        h.setParent(u),
  ...
  createWatermark() {
    ...
    let a = new L(100, 100, 1),
      r = new v({ map: t, transparent: !0, opacity: 0.15 }),
      e = new b(a, r);
    (e.position.x -= 10),
      (e.position.y -= 10),
      (e.position.z -= 10),
